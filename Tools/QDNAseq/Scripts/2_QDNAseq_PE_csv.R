...
input <- read.csv("/home/minerva/WGS/QDNAseq/Data/MaNiLa/MaNiLa_CS3_input_ichorCNA.csv", header=T, sep=",", stringsAsFactors = F)
...
# 1. Copy Number Segmentation using circular binary segmentation (CBS) algorithm in DNAcopy
...
copyNumbersCalled <- callBins(copyNumbersSegmented, organism="human", method="cutoff") #for non-cancer or very little CNA samples when "CGHcall" method fitting fail which is designed for tumor sample.
