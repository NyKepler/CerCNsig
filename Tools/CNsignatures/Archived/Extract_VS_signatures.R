...
VS_components <- readRDS("/home/minerva/WGS/CNsignatures/Data/MaNiLa/CerCNsig_All_v2/HGSC.VS.all.CerCNsig.Components.rds")
...
coefmap(sigs, Colv="consensus",tracks=c("consensus:"), main="Sample x Signature matrix Coefmap")#annCol=annCol,
dev.off
...
sig.melt <- within(sig.melt, ID <- factor(ID, c('s1','s2','s3','s4','s5','s6'), ordered = TRUE))

p<-ggplot(sig.melt, aes(x = variable, y = value, fill = ID)) + 
  geom_bar(stat = 'identity') + xlab("") + ylab("Exposures") + 
  ylim(0, 1) + guides(fill=guide_legend(title = "CerCNsig")) + 
...
table[2,] <- gsub("HGSOC_", "", table[2,])
#table[2,] <- gsub("BRCA_", "", table[2,])
...
n <- nrow(matched)
pdf(file = paste0(type, ".Signature.table.pdf"), onefile = FALSE, width = 12, height = 10)
